...
class id:
	...
	def connect_d(self):
		...
		v=len(rg1)
		i=-1	
		if combo1.get() == self.type:# + ' ' + self.b:
			while i < v-1:
				i=i+1
				if re.search(self.adr, rg1[i]):
					...
					if a4 in ('34401A', '34410A', '34411A', '34460A', '34461A', '34465A', '34470A'): 
						...
						lb.see(END)
					if a4 in ('33622A', 'N5183A'): 
						a10.set('Генератор ' + a4 + ' ' + 'подключен')
						lb.insert(END, a10.get())
						lb.see(END)					
	...
	def connect_f(self):
		...
		v=len(rg1)
		i=-1		
		if combo2.get() == self.type:# + ' ' + self.b:
			while i < v-1:
				i=i+1
				if re.search(self.adr, rg1[i]):
...
def connect_dmm():
	USB1 = id('34411A', r'\b0x0A07\b', 'usb', b10)
	USB2 = id('34461A', r'\b0x1401\b', 'usb', b10)
	USB3 = id('33622A', r'\b0x5707\b', 'usb', b10)
	USB4 = id('N1913A', r'\b0x5418\b', 'usb', b10)
	USB5 = id('34461A', r'\b0x1301\b', 'usb', b10)
	COM1 = id('COM1', r'\bASRL1\b', 'com', b13)
	COM2 = id('COM2', r'\bASRL2\b', 'com', b13)
	COM3 = id('COM3', r'\bASRL3\b', 'com', b13)
	COM11 = id('COM11', r'\bASRL11\b', 'com', b13)
	USB1.connect_d()
	USB2.connect_d()
	USB3.connect_d()
	USB4.connect_d()
...
def start():
	...
	if a4 in ('34465A', '34470A'):
		...
		#~10V,200kHz
		if b4 == '5522A':
			thread = call('ac', '1', 'OUT 1 V, 200 kHz', 'CONF:VOLT:AC 10.0', 'D162', 'E162', 'DET:BAND 3', '8', '2')
			thread = call('ac', '3', 'OUT 3 V, 200 kHz', 'CONF:VOLT:AC 10.0', 'D163', 'E163', 'DET:BAND 3', '8', '1.333')
			thread = call('ac', '5', 'OUT 5 V, 200 kHz', 'CONF:VOLT:AC 10.0', 'D164', 'E164', 'DET:BAND 3', '8', '1.2')
			thread = call('ac', '7', 'OUT 7 V, 200 kHz', 'CONF:VOLT:AC 10.0', 'D165', 'E165', 'DET:BAND 3', '8', '1.143')
			thread = call('ac', '10', 'OUT 10 V, 200 kHz', 'CONF:VOLT:AC 10.0', 'D166', 'E166', 'DET:BAND 3', '8', '1.1')
		#~100V,200kHz
		if b4 == '5522A':
			thread = call('ac', '10', 'OUT 10 V, 200 kHz', 'CONF:VOLT:AC 100', 'D167', 'E167', 'DET:BAND 3', '8', '2')
			thread = call('ac', '30', 'OUT 30 V, 200 kHz', 'CONF:VOLT:AC 100', 'D168', 'E168', 'DET:BAND 3', '8', '1.333')
			thread = call('ac', '50', 'OUT 50 V, 200 kHz', 'CONF:VOLT:AC 100', 'D169', 'E169', 'DET:BAND 3', '8', '1.2')
			thread = call('ac', '70', 'OUT 70 V, 200 kHz', 'CONF:VOLT:AC 100', 'D170', 'E170', 'DET:BAND 3', '8', '1.143')
			thread = call('ac', '100', 'OUT 100 V, 200 kHz', 'CONF:VOLT:AC 100', 'D171', 'E171', 'DET:BAND 3', '8', '1.1')
		#5Hz
		...
		thread = call('dci', '3.0', 'OUT 3.0 A', 'CONF:CURR:DC 3.0', 'C200', 'D200', 'DET:BAND 20', '5', '0.22')
		thread = reset()
		#~0.0001A, 20Hz
		...
		thread = call('aci', '3.0', 'OUT 3.0 A, 1 kHz', 'CONF:CURR:AC 3.0', 'D256', 'E256', 'DET:BAND 20', '5', '0.27')	
		#~0.0001A, 5kHz
		...
		if b4 == '5522A':
			thread = call('aci', '3.0', 'OUT 3.0 A, 5 kHz', 'CONF:CURR:AC 3.0', 'D287', 'E287', 'DET:BAND 20', '5', '0.27')	
		#10A
		thread = message('Переключите красный провод на контакт 10А мультиметра')
		thread = reset()
		...
		thread = message('Переключите провода по двухпроводной схеме\n для измерения сопротивления')
		thread = reset()
		thread = call('res2', '1000000', 'OUT 1 MOHM; ZCOMP WIRE2', 'CONF:FRES 1 MOHM', 'C322', 'D322', 'DET:BAND 20', '5', '0.008')
...
ttk.Style().configure("BW.TLabel", padding=6, font='arial 10', foreground="black", background="FloralWhite")
...
lbf1.place(x=5, y=58)
lbf2 = ttk.LabelFrame(root, text='Условия калибровки', width=475, height=80)
lbf2.place(x=5, y=250)
lbf3 = ttk.LabelFrame(root, text='Прогресс', width=475, height=50)
lbf3.place(x=5, y=335)

but1 = ttk.Button(root, text = 'Connect DMM', width = 16, command=connect_dmm, style='TButton')
but1.place(x=10,y=75)
but2 = ttk.Button(root, text = 'Connect Fluke', width = 16, command=connect_fluke)
but2.place(x=10,y=115)
but3 = ttk.Button(root, text = 'СТАРТ', width = 16, command=start)
but3.place(x=575,y=360)

lab1 = ttk.Label(root, text='ID DMM:', style="BW.TLabel")
lab1.place(x=47,y=150)
lab2 = ttk.Label(root, text='ID Fluke:', style="BW.TLabel")
lab2.place(x=47,y=175)
lab3 = ttk.Label(root, text='Название протокола:', style="BW.TLabel")
lab3.place(x=7,y=205)
lab4 = ttk.Label(root, text='Температура:', style="BW.TLabel")
lab4.place(x=7,y=265)
lab5 = ttk.Label(root, text='Влажность:', style="BW.TLabel")
lab5.place(x=100,y=265)
lab6 = ttk.Label(root, text='Давление:', style="BW.TLabel")
lab6.place(x=190,y=265)
lab7 = ttk.Label(root, text='Заказчик:', style="BW.TLabel")
lab7.place(x=275,y=265)
lab8 = ttk.Label(root, text='Поверитель:', style="BW.TLabel")
lab8.place(x=360,y=265)

combo1 = ttk.Combobox(root, state='readonly', height=4, width=50)
combo1.place(x=145,y=82)
combo2 = ttk.Combobox(root, state='readonly', height=4, width=50)
...
progress1 = ttk.Progressbar(root, orient='horizontal', mode='determinate', length = 470, value = 0)
progress1.place(x=7, y=355)
#progress2 = ttk.Progressbar(root, orient='horizontal', mode='indeterminate', length = 470)
#progress2.place(x=7, y=310)
